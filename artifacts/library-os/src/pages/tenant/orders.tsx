                    ...
                    return (
                      <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-muted-foreground">{order.studentPhone}</div>
                        </td>
                        <td className="px-6 py-4 text-muted-foreground">
                          {new Date(order.createdAt).toLocaleDateString("ar-JO")}
                        </td>
                        <td className="px-6 py-4 font-bold text-primary">
                          {order.total.toFixed(3)} د.أ
                        </td>
                        <td className="px-6 py-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-gray-50 rounded-xl border border-border/50">
                  ...
                  <p className="font-bold">{selectedOrder.studentName}</p>
                  <p className="text-sm text-muted-foreground">{selectedOrder.studentPhone}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-border/50">
              ...
              <div className="border border-border/50 rounded-xl overflow-hidden">
                ...
                <div className="divide-y divide-border/40">
                  {selectedOrder.items?.map((item, i) => (
                    <div key={i} className="px-4 py-3 flex justify-between items-center">
                      ...
                      <p className="font-bold text-primary">{item.total.toFixed(3)} د.أ</p>
                    </div>
                  ))}
                </div>
                <div className="px-4 py-3 bg-gray-50/70 border-t border-border/50 flex justify-between items-center">
                  ...
                  <p className="font-bold text-lg text-primary">{selectedOrder.total.toFixed(3)} د.أ</p>
                </div>
              </div>
            </div>
          )}
