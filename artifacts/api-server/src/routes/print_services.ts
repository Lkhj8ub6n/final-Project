...
router.get("/print-services", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
...
router.post("/print-services", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
...
router.patch("/print-services/:serviceId", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
...
router.delete("/print-services/:serviceId", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.serviceId) ? req.params.serviceId[0] : req.params.serviceId, 10);
