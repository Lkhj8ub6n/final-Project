...
router.get("/products", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
  const { category, search, showInStore, lowStock, limit, offset } = req.query as any;
  ...
  const [totalCount] = await db.select({ count: sql<number>`count(*)` }).from(productsTable).where(and(...conditions));
  const products = await db.select().from(productsTable).where(and(...conditions)).orderBy(productsTable.name).limit(parsedLimit).offset(parsedOffset);
  
  res.setHeader('X-Total-Count', totalCount.count.toString());
  res.setHeader('X-Page-Limit', parsedLimit.toString());
  res.setHeader('X-Page-Offset', parsedOffset.toString());
  res.setHeader('Access-Control-Expose-Headers', 'X-Total-Count, X-Page-Limit, X-Page-Offset');
  
  res.json(products.map(formatProduct));
}));

router.post("/products", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
...
router.get("/products/barcode/:barcode", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
...
router.get("/products/:productId", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
...
router.patch("/products/:productId", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
  const id = parseInt(Array.isArray(req.params.productId) ? req.params.productId[0] : req.params.productId, 10);
  const updates: any = {};
...
router.delete("/products/:productId", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
  const id = parseInt(Array.isArray(req.params.productId) ? req.params.productId[0] : req.params.productId, 10);
...
router.post("/products/:productId/toggle-store", authenticate as any, asyncHandler(async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
  const id = parseInt(Array.isArray(req.params.productId) ? req.params.productId[0] : req.params.productId, 10);
