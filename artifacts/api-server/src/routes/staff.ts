...
router.get("/staff", authenticate as any, requireRole("tenant_admin") as any, async (req: AuthRequest, res): Promise<void> => {
...
router.post("/staff", authenticate as any, requireRole("tenant_admin") as any, async (req: AuthRequest, res): Promise<void> => {
  const tenantId = req.user!.tenantId;
  if (!tenantId) { res.status(403).json({ error: "No tenant" }); return; }
...
router.patch("/staff/:staffId", authenticate as any, requireRole("tenant_admin") as any, async (req: AuthRequest, res): Promise<void> => {
...
router.delete("/staff/:staffId", authenticate as any, requireRole("tenant_admin") as any, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.staffId) ? req.params.staffId[0] : req.params.staffId, 10);
...
router.post("/staff/:staffId/toggle-status", authenticate as any, requireRole("tenant_admin") as any, async (req: AuthRequest, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.staffId) ? req.params.staffId[0] : req.params.staffId, 10);
